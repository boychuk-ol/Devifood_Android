...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $delete_courier = $dbCourier->deleteCourier($column_name, $value, $condition_type, $value2);
        if ($delete_courier) {
            $response['error'] = false;
            $response['message'] = 'Courier deleted successfully';
        } else {
            $response['error'] = true;
            $response['message'] = 'Courier not deleted or does not exist';
        }
    } catch (Exception $e) {
        $response['error'] = true;
