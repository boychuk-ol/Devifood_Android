...
require_once(dirname(__FILE__, 3).'/repos/DbCategoryRepository.php');
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        if (isset($_GET['created_time'])) {
            ...
            $orders = $dbOrder->getOrdersByCreatedTime($created_time);
            if ($orders) {
                ...
                foreach ($orders as $order) {
                    ...
                    $order['courier'] = $courier ? $courier : null;
                    $order['client'] = $client ? $client : null;
                    $order['shop_location'] = $shop_location ? $shop_location : null;
                    if ($order['shop_location'] && $order['shop_location']['shop_id']) {
                        ...
                        if ($shop) {
                            ...
                            $shop['image'] = $shop_image ? $shop_image : null;
                            ...
                            if ($shop_category) {
                                ...
                                $shop_category['image'] = $category_image ? $category_image : null;
                                unset($shop_category['FK_image_id']);
                            }
                            $shop['category'] = $shop_category ? $shop_category : null;
                            unset($shop['FK_category_id']);
                        }
                        $order['shop_location']['shop'] = $shop;
                        unset($order['shop_location']['shop_id'], $order['shop_location']['client_id']);
                    }
                    $order['client_location'] = $client_location ? $client_location : null;
            ...
            $response['error'] = true;
            $response['message'] = 'Missing parameter: created_time';
        }
    } else {
        $response['error'] = true;
