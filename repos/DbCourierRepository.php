...
class DbCourierRepository {
    ...
    function createCourier($courier) {
        $stmt = $this->con->prepare("INSERT INTO `client`(`full_name`, `phone`, `email`, `work_region`, `work_area`) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sss", $courier->full_name, $courier->phone, $courier->email, $courier->work_region, $courier->work_area);
        $stmt->execute();
        $response = $stmt->get_result();  
