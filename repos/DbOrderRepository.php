...
class DbOrderRepository {
    ...
    function createOrder($order) {
        $stmt = $this->con->prepare("INSERT INTO orders (`created`, `o_status`, `delivery_price`, `delivery_time`, 
                                    `total_price`, `payment_method`, `FKclient_id`, `FKcourier_id`, 
                                    `shop_location_id`, `client_location_id`) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
        $stmt->bind_param("isdidsiiii", $order->created, $order->o_status, $order->delivery_price, $order->delivery_time, $order->total_price, 
                                        $order->payment_method, $order->FKclient_id, $order->FKcourier_id, $order->shop_location_id, $order->client_location_id);
        $stmt->execute();
    }
    ...
    private function getColumnType($column_name) {
        $query = "SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'orders' AND COLUMN_NAME = ?";
        $stmt = $this->con->prepare($query);
    ...
    function getOrderByCondition($condition_column, $condition_value, $condition_type, $condition_value2 = null) {
        $query = "SELECT * FROM orders WHERE {$condition_column} ";
